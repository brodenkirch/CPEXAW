...
def plot_CFAD(cfad_array, contours, colorbar_label, save_label, var_label, var_centers_meshgrid, height_centers_meshgrid, var_edges, case_dict, median_height_profile_above1500m, medianDBZ_profile_above1500m, Q1DBZ_profile_above1500m, Q3DBZ_profile_above1500m, dbz_plot = True):
    ...
    #save the figure
    plt.savefig(''.join(['/Users/brodenkirch/Desktop/CPEX-CV_CFAD_plots_TO_BE_SORTED/', save_label]), bbox_inches = 'tight')
    plt.close()
...
###########################################################################################################################                       
                    ...
                    # total_apr_profiles += 25
                    
                    # #choose the "nadir" ray factoring in aircraft roll
                    # ac_roll = np.nanmean(roll[start_time_idx:end_time_idx + 1,:], axis = 1)  #roll varies slightly w/ray, so take the average roll value for each scan and use that for ray adjustment
                    ...
                    # for ac_roll_value in ac_roll:
                    #     if abs(ac_roll_value) >= 10:
                    #         apr_profile_roll10 += 1
                    
                    prof_height3d = alt3d[start_time_idx:end_time_idx + 1,:,:].values
                    prof_height = prof_height3d.reshape(prof_height3d.shape[0], -1)  #convert the 3-D array to a 2-D array for Pandas; -1 means that numpy calculates the necessary column length (in this case, prof_height.shape[1] * prof_height.shape[2])
                    
                    prof_dbz3d = ku_data[start_time_idx:end_time_idx + 1,:,:].values
                    prof_dbz = prof_dbz3d.reshape(prof_dbz3d.shape[0], -1)           #convert the 3-D array to a 2-D array for Pandas; -1 means that numpy calculates the necessary column length (in this case, prof_dbz.shape[1] * prof_dbz.shape[2])
                    
                    # #grab the height/dbz data above 1.5km
                    # idx_1500m_array = np.argmin(np.abs(prof_height3d - 1500), axis = 2)    #shape (numscans, numrays), so it calculates the idxmin for each ray of each scan
                    
                    # prof_heights_above1500m_temp_df = pd.DataFrame()
                    # prof_dbzs_above1500m_temp_df = pd.DataFrame()
                    # for scan in range(idx_1500m_array.shape[0]):
                    #     for ray in range(idx_1500m_array.shape[1]):
                    #         idx_1500m = idx_1500m_array[scan, ray]
                    #         prof_height_above1500m = np.flip(prof_height3d[scan, ray, :idx_1500m + 1])  #height profile above 1.5km, in ascending order
                    #         prof_dbz_above1500m = np.flip(prof_dbz3d[scan, ray, :idx_1500m + 1])  #Ku-band profile above 1.5km, in ascending order
                            
                    #         prof_heights_above1500m_temp_df = pd.concat((prof_heights_above1500m_temp_df, pd.Series(prof_height_above1500m)), axis = 1)
                    #         prof_dbzs_above1500m_temp_df = pd.concat((prof_dbzs_above1500m_temp_df, pd.Series(prof_dbz_above1500m)), axis = 1)
                    
                    #store all the profile's height/dbz data in Pandas DataFrames
                    if new_ku_array:  #if this is the first qualifying time and ray of the first APR file of the first case in case_dict
                        height_concat = pd.DataFrame(prof_height)
                        dbz_concat = pd.DataFrame(prof_dbz)
                        new_ku_array = False
                    
                        # #create a DataFrame of values above 1.5km, in order to calculate median/quartile reflectivity profile for the given case
                        #     #NOTE: WE ARE ASSUMING THAT EACH ROW CORRESPONDS TO A NEAR IDENTICAL HEIGHT LEVEL
                        #         #this is the best we can do, and should be fine given that each profile is starting from ~1500m and the height resolution is the same for each profile     
                        # height_profiles_above1500m_df = pd.DataFrame(prof_heights_above1500m_temp_df)  #height profile from 1500m to top of profile
                        # dbz_profiles_above1500m_df = pd.DataFrame(prof_dbzs_above1500m_temp_df)        #dbz profile from 1500m to top of profile
                    else:
                        height_concat = pd.concat((height_concat, pd.DataFrame(prof_height)), axis = 0, ignore_index = True)  #add profile as a new row to the df; differences in profile lengths are filled in with NaNs
                        dbz_concat = pd.concat((dbz_concat, pd.DataFrame(prof_dbz)), axis = 0, ignore_index = True)  #add profile as a new row to the df; differences in profile lengths are filled in with NaNs
                    
                        # #NOTE: WE ARE ASSUMING THAT EACH ROW CORRESPONDS TO A NEAR IDENTICAL HEIGHT LEVEL
                        #     #this is the best we can do, and should be fine given that each profile is starting from ~1500m and the height resolution is the same for each profile
                        # height_profiles_above1500m_df = pd.concat((height_profiles_above1500m_df, prof_heights_above1500m_temp_df), axis = 1, ignore_index = True)  #add profile as a new column to the df; differences in profile lengths are filled in with NaNs
                        # dbz_profiles_above1500m_df = pd.concat((dbz_profiles_above1500m_df, prof_dbzs_above1500m_temp_df), axis = 1, ignore_index = True)  #add profile as a new column to the df; differences in profile lengths are filled in with NaNs
                    
                    if vel_good:
                        prof_vel3d = vel_data[start_time_idx:end_time_idx + 1,:,:].values
                        prof_vel = prof_vel3d.reshape(prof_vel3d.shape[0], -1)  #convert the 3-D array to a 2-D array for Pandas; -1 means that numpy calculates the necessary column length (in this case, prof_vel.shape[1] * prof_vel.shape[2])
                    
                        #store each profile's velocity data in Pandas DataFrames if 
                        #the profile has Ku data > 0 dBZ above 1.5km (i.e. omits clear profiles)
                    
                        #if np.nanmax(prof_dbz_above1500m) > 0:  #if you want to check this condition, 
                                                                 #then do this whole "if vel_good:" block of code within the 
                                                                 #"idx_1500m = idx_1500m_array[scan, ray]" loop above. 
                                                                 #Otherwise this is VERY inefficient and VERY VERY SLOW!
                        if True:
                            if new_vel_array:  #if this is the first qualifying time and ray of the first APR file of the first case in case_dict
                                height_vel_concat = pd.DataFrame(prof_height)
                                vel_concat = pd.DataFrame(prof_vel)
                                new_vel_array = False
                            else:
                                height_vel_concat = pd.concat((height_vel_concat, pd.DataFrame(prof_height)), axis = 0, ignore_index = True)  #add profile as a new row to the df; differences in profile lengths are filled in with NaNs
                                vel_concat = pd.concat((vel_concat, pd.DataFrame(prof_vel)), axis = 0, ignore_index = True)  #add profile as a new row to the df; differences in profile lengths are filled in with NaNs
                        else:  #clear profiles (which may have noisy velocity data) are omitted from the velocity CFAD
                            pass
                    else:
                        pass
...
###########################################################################################################################                            
    ...
    median_height_profile_above1500m = None
    medianDBZ_profile_above1500m = None
    Q1DBZ_profile_above1500m = None
    Q3DBZ_profile_above1500m = None
    ...
    cfad_array, xedges, yedges = np.histogram2d(dbz_concat.to_numpy().flatten(), height_concat.to_numpy().flatten(), bins = (dbz_edges,height_edges))
    ...
    plot_CFAD(cfad_array, contours, colorbar_label, save_label, var_label, dbz_centers_meshgrid, height_centers_meshgrid, dbz_edges, case_dict, median_height_profile_above1500m, medianDBZ_profile_above1500m, Q1DBZ_profile_above1500m, Q3DBZ_profile_above1500m, dbz_plot = False)
    ...
    cfad_array_vel, xedges, yedges = np.histogram2d(vel_concat.to_numpy().flatten(), height_vel_concat.to_numpy().flatten(), bins = (vel_edges,height_edges))
